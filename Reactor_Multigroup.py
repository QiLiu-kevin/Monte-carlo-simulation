...
class Geometry:
    ...
    def spawn_initial_neutrons(self, N):
        ...
        return neutron_seq
    def spawn_initial_neutrons_r(self, N):
        """
        depend on the geometry function and energy distribution
        """
        neutron_seq = []
...
def radius_distri(neutron_seq,title=time.perf_counter()):
    ...
    ax1.hist(path1_dis, 50, alpha=0.75)
    title = time.perf_counter()
    fig1.suptitle(str(title), fontsize=16)
    plt.show()
    
def radius_distri_n(neutron_seq,title=time.perf_counter()):
    path1_dis = []
    for i in neutron_seq:
        path1_dis.append(np.linalg.norm(i.position))
    fig1, ax1 = plt.subplots(1, 1)
    n, bins = ax1.hist(path1_dis, 50, alpha=0.75)
    title = time.perf_counter()
    fig1.suptitle(str(title), fontsize=16)
    plt.show()
""" Define cycles """
...
#%%
""" Define one group approximation """ 

energies = np.linspace(0,10000000,100001)
...
for i in energies:
    XS = np.hstack((XS,u235[2].xs['0K'](i)))
end = time.perf_counter()
print("time: " + str(end - start))

f = np.vstack((energies,XS))
def function(energy):
    while True:
        if energy<10000000:
            return XS[int(np.around(energy/100))]
        else:
            return XS[int(np.around(10000000/100))]

start = time.perf_counter()
for i in range(1000000):
    function(watts_distribution())
end = time.perf_counter()
print("time: " + str(end - start))

start = time.perf_counter()
for i in range(1000000):
    u235[2].xs['0K'](watts_distribution())
end = time.perf_counter()
print("time: " + str(end - start))
...
pickle_off = open("/Users/a1234/Documents/Programming/Python/pickle/cycles_0.81_pure.pickle2","rb")
cyclesa = pickle.load(pickle_off)
pickle_off = open("/Users/a1234/Documents/Programming/Python/pickle/10000n_50cycles_u235_5%_u238_95%_UO2sphere0.134.pickle","rb")
cyclesb = pickle.load(pickle_off)
pickle_off = open("/Users/a1234/Documents/Programming/Python/pickle/10000n_20cycles_u235_5%_u238_95%sphere0.255.pickle","rb")
cyclesc = pickle.load(pickle_off)
#%%
""" Examine the equilibrium """



generation_0 = cycles[0]
generation_z = cycles[-1]
...
for i in cycles[-1]:
    path1_dis.append(np.linalg.norm(i.position))
fig1, ax1 = plt.subplots(1, 1)
n, bins, patches = ax1.hist(path1_dis, 20,  alpha=0.75)
title = time.perf_counter()
fig1.suptitle(str(title), fontsize=16)
plt.show()
...
for i in range(len(n)):
    x_value = (bins[i+1]+bins[i])/2
    nnnnn.append(n[i]/(4*np.pi*x_value**2))

#


fig2, ax2 = plt.subplots(1, 1)
ax2.bar(bins[:-1],nnnnn, width=np.diff(bins), edgecolor="black", align="edge")
title = time.perf_counter()
fig2.suptitle(str(title), fontsize=16)
plt.show()
#%%
def avg_radius(cycles_1,n):
    freqs = np.zeros((0,n))
    for i in cycles_1:
        path1_dis = np.array([])
        for ii in i:
            path1_dis = np.hstack((path1_dis,np.linalg.norm(ii.position)))
        frq, bins = np.histogram(path1_dis, n)
        freqs = np.vstack((freqs,frq))
        
    return freqs, bins


def histo(cycles_1,rad,n,number):

    fig2, ax2 = plt.subplots(1, 1)
    ax2.bar(bins[:-1],avg_radius(cycles[90:],20)[0].mean(0), width=np.diff(bins), edgecolor="black", align="edge",alpha=0.75)
    title = "Neutron distribution "


    xxx = np.linspace(0,rad,100000)
    A = number/(4/3*np.pi*rad**3)/n*rad
    yyy = 4*np.pi*xxx**2*A
    line, = ax2.plot(xxx,yyy,color="k")
    ...
    fig2.suptitle(str(title), fontsize=16)
    plt.savefig('U235_100%sphere_neutron_distribution.pdf')
    plt.show()


def avg_radius_n(cycles_1,rad,n):
    freqs = np.zeros((0,n))
    for i in cycles_1:
        path1_dis = np.array([])
        for ii in i:
            path1_dis = np.hstack((path1_dis,np.linalg.norm(ii.position/rad)))
        frq, bins = np.histogram(path1_dis, n, density = True)
        freqs = np.vstack((freqs,frq))
        
    return freqs, bins

def histo_n(cycles_1,rad,n,number):
    freqs = np.zeros((0,n))
    for i in cycles_1:
        path1_dis = np.array([])
        for ii in i:
    ...
    fig2, ax2 = plt.subplots(1, 1)
    ax2.bar(bins[:-1],freqs.mean(0), width=np.diff(bins), edgecolor="black", align="edge",alpha=0.75)
    title = "Neutron distribution "
...
histo_n(cyclesa,0.081,20,10000)
histo_n(cyclesb,0.134,20,10000)
histo_n(cyclesc,0.255,20,10000)
